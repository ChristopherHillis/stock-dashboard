...
export default function Gainers() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setData(json.gainers); // already sliced & shaped in the API
    }
    load();
  }, []);
  ...
  return (
    <div className="space-y-2">
      {data.slice(0, 5).map((s) => {
        // Normalize everything to numbers
        ...
        const percentText = changePercent.toFixed(2); // already in percent, not 0.x
